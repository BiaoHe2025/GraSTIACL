...
from sklearn.metrics import precision_recall_curve, average_precision_score, roc_curve, auc, precision_score, \
    recall_score, f1_score, confusion_matrix, accuracy_score, roc_auc_score
import time


def get_emb_y(loader, encoder, beta, device, dtype='numpy', is_rand_label=False):
    # train_emb, train_y
    x, y = encoder.get_embeddings(loader, beta, device, is_rand_label)
...
class EmbeddingEvaluation():
    ...
    def embedding_evaluation(self, encoder, beta, train_loader, valid_loader, test_loader, flag):
        encoder.eval()
        if flag:
            val_start = time.time()
        train_emb, train_y = get_emb_y(train_loader, encoder, beta, self.device, is_rand_label=self.is_rand_label)
        val_emb, val_y = get_emb_y(valid_loader, encoder, beta, self.device, is_rand_label=self.is_rand_label)
        test_emb, test_y = get_emb_y(test_loader, encoder, beta, self.device, is_rand_label=self.is_rand_label)
        if flag:
            val_end = time.time()
        ...
        train_pre_score = precision_score(train_y, train_raw, zero_division=0)
        val_pre_score = precision_score(val_y, val_raw, zero_division=0)
        test_pre_score = precision_score(test_y, test_raw, zero_division=0)

        test_auc_score = roc_auc_score(test_y, test_raw)
        return train_score, val_score, test_score, train_f1_score, val_f1_score, test_f1_score, train_sen_score, val_sen_score, test_sen_score, train_spe_score, val_spe_score, test_spe_score, train_pre_score, val_pre_score, test_pre_score, test_auc_score, running_time

    def kf_embedding_evaluation(self, encoder, beta, dataset, folds=5, batch_size=64, flag=True):
        kf_train = []
        kf_val = []
        kf_test = []
        kf_train_f1 = []
        ...
        for k_id, (train_val_index, test_index) in enumerate(kf.split(dataset)):
            ...
            train_score, val_score, test_score, train_f1, val_f1, test_f1, train_sen, val_sen, test_sen, train_spe, val_spe, test_spe, train_pre, val_pre, test_pre, test_auc, running_time = self.embedding_evaluation(
                encoder, beta, train_loader, valid_loader, test_loader, flag)
            ...
            kf_train_pre.append(train_pre)
            kf_val_pre.append(val_pre)
            kf_test_pre.append(test_pre)
        ...
        kf_test_ms = [np.array(kf_test).mean(), np.array(kf_test).std(), np.array(kf_test_f1).mean(),
                      ...
                      np.array(kf_test_pre).mean(), np.array(kf_test_pre).std(), np.array(kf_test_auc).mean(),
                      np.array(kf_test_auc).std()]
